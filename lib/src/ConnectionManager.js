...
class Manager {
    ...
    connect(connect = true, url = this.url, optionalApis = {enableCrypto: false, enableOrders: false}) {
        return new Promise((resolve, reject) => {
            Apis.instance(url, connect, undefined, optionalApis, this._onClose.bind(this)).init_promise
            .then((res) => {
                this.url = url;
    ...
    connectWithFallback(connect = true, url = this.url, index = 0, resolve = null, reject = null, optionalApis) {
        ...
        const fallback = (err, resolve, reject) => {
            ...
            return this.connectWithFallback(connect, this.urls[index], index + 1, resolve, reject, optionalApis);
        }
        if (resolve && reject) {
            return this.connect(connect, url, optionalApis)
            .then(resolve)
            .catch((err) => {
            ...
            return new Promise((resolve, reject) => {
                this.connect(connect, undefined, optionalApis)
                .then(resolve)
                .catch((err) => {
                    fallback(err, resolve, reject);
                })
